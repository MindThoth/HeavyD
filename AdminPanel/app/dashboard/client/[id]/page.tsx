...
export default function ClientPage() {
  ...
  const loadClient = async () => {
    try {
      ...
      // Load brief content for boat-lettering clients (boat name comes from Master sheet column X)
      if (response.client.service === 'boat-lettering' && response.client.briefLink) {
        let briefId = ''
        if (response.client.briefLink.includes('?id=')) {
          briefId = response.client.briefLink.split('?id=')[1]?.split('&')[0] || ''
        } else if (response.client.briefLink.includes('/d/')) {
          briefId = response.client.briefLink.split('/d/')[1]?.split('/')[0] || ''
        }
        if (briefId) loadBriefContent(briefId)
      }
      ...
      <Tabs defaultValue="timesheet" className="space-y-6">
        ...
        <TabsContent value="details" className="space-y-6">
          {loadingBrief ? (
            ...
            <div className="space-y-6">
              {/* Boat Name Header */}
              {(briefData.BoatName || client?.boatName) && (
                ...
                <CardContent className="space-y-3">
                  {(briefData.BoatName || client?.boatName) && (
                    <div>
                      ...
                      <p className="font-medium mt-1 text-blue-600">{briefData.BoatName || client?.boatName}</p>
                    </div>
                  )}
                  {(briefData.BoatType || briefData.Type || briefData.Model) && (
                    <div>
