                      ...
                      <td className="px-2 sm:px-4 py-3">
                        <Link href={`/dashboard/client/${client.revisionCode || client.accessCode}`}>
                          <div className="cursor-pointer hover:text-blue-600">
                            ...
                            {(function() {
                              const isBoat = (client.service || "").toLowerCase().replace(/\s+/g, "-") === "boat-lettering";
                              const sub = isBoat ? (client.boatName || "").trim() : (client.company || "").trim();
                              return sub ? <p className="text-xs sm:text-sm text-gray-500">{sub}</p> : null;
                            })()}
                          </div>
                        </Link>
                      </td>
