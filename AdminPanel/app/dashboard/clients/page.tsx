...
import { clientsApi, clientExtrasApi, Client } from "@/lib/api"
...
export default function ClientsPage() {
  ...
  const loadClients = async () => {
    try {
      ...
      // Load boat names for boat-lettering clients
      loadBoatNames(clientsData)
    } catch (error: any) {
  ...
  const loadBoatNames = async (clientsData: Client[]) => {
    const boatLetteringClients = clientsData.filter(
      (client) => client.service === 'boat-lettering' && client.briefLink
    )

    const boatNamePromises = boatLetteringClients.map(async (client) => {
      try {
        // Extract document ID from briefLink
        let briefId = ''
        if (client.briefLink.includes('?id=')) {
          briefId = client.briefLink.split('?id=')[1]?.split('&')[0]
        } else if (client.briefLink.includes('/d/')) {
          briefId = client.briefLink.split('/d/')[1]?.split('/')[0]
        }

        if (briefId) {
          const response = await clientExtrasApi.getBoatName(briefId)
          if (response.success && response.boatName) {
            return { code: client.accessCode, name: response.boatName }
          }
        }
      } catch (error) {
        console.error(`Error loading boat name for ${client.name}:`, error)
      }
      return null
    })

    const results = await Promise.all(boatNamePromises)
    const boatNamesMap: Record<string, string> = {}
    results.forEach((result) => {
      if (result) {
        boatNamesMap[result.code] = result.name
      }
    })
    setBoatNames(boatNamesMap)
  }
                    ...
                    <tr key={client.accessCode} className="hover:bg-gray-50">
                      ...
                      <td className="px-2 sm:px-4 py-3 hidden sm:table-cell">
                        <div>
                          ...
                          {client.service === 'boat-lettering' && boatNames[client.accessCode] && (
                            <p className="text-xs text-blue-600 font-medium mt-1">
                              {boatNames[client.accessCode]}
                            </p>
                          )}
                        </div>
                      </td>
