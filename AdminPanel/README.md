...
The Admin Panel talks to the script in **`../GoogleScript/adminpanel.gs`** (same repo).

1. Go to https://script.google.com
2. Create a new project (or open the one you use for the admin API)
3. Copy the full code from **`GoogleScript/adminpanel.gs`** into the project
4. Deploy as web app:
   ...
   - Click "Deploy"
   - Copy the web app URL and add it to `.env.local` as `NEXT_PUBLIC_GAS_ENDPOINT` (see `.env.example` for the current deployment URL)
...
**Use the full script:** Copy everything from **`../GoogleScript/adminpanel.gs`** into your Apps Script project (it includes the web app API and the spreadsheet menu: clients, expenses, timesheets, employees, receipts, estimation, quote, etc.). The simplified snippet below is for reference only.
