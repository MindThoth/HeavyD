...
Use **one** Google Apps Script project with all `.gs` files. Deploy it **once** as a web app. That single URL is used for:

| Use | How |
|-----|-----|
| **Website** (heavydetailing.com) form | POST to the script URL (no `api=admin`) |
| **Dashboard** (clients, login, etc.) | GET/POST with `action=...` (no `api=admin`) |
| **Accept Quote** link in emails | GET with `action=acceptEmail` |
| **Admin Panel** (admin.heavydetailing.com) | GET/POST with **`api=admin`** – main.gs routes to adminpanel.gs |

**Deployment ID (your one script URL):**  
`AKfycbxidAdAasl0tFNMk2ILW_44gXAxVVTYPyzfwRBum9XTuZ9wOC5_BjcRKCALJf0IKD2h-g`

- **Website** and **Admin Panel** both set `NEXT_PUBLIC_GAS_ENDPOINT` to that same URL (see `Website/.env.example` and `AdminPanel/.env.example`). The Admin Panel app automatically adds `api=admin` to every request so main.gs sends them to the Admin Panel handlers.
- Only **main.gs** defines `doGet` and `doPost`. Other files use distinct names (`handleAdminGet`, `handleWebsiteFormSubmission`, etc.) so there are no conflicts when everything is in one project.
...
### main.gs
**Purpose**: Single entry point; routes all GET/POST to website.gs, dashboard.gs, etc.

- `doGet(e)` → website health or `handleDashboardGet(e)` for dashboard actions (including `acceptEmail`).
- `doPost(e)` → `handleWebsiteFormSubmission(e)` for form submissions, or `handleDashboardPost(e)` for dashboard actions.

All other .gs files are used by this router (same project).
...
**Used by**: main.gs (routeToWebsite_POST). Not deployed alone.
